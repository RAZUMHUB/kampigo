...
def text_similarity(text_a: str, text_b: str, text_model) -> float:
    from sentence_transformers import util  # lazy import - heavy dependency, not needed by pure functions above

    if not text_a.strip() or not text_b.strip():
        return 0.0
    vec_a = text_model.encode(text_a, normalize_embeddings=True, convert_to_tensor=True)
    vec_b = text_model.encode(text_b, normalize_embeddings=True, convert_to_tensor=True)
